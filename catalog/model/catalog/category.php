...
class ModelCatalogCategory extends Model {
    ...
    public function getManufacturerCategories($manufacturer_id){
        $query = $this->db->query("SELECT DISTINCT c.category_id AS category_id,cd.name AS name FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) LEFT JOIN " . DB_PREFIX . "category_to_store c2s ON (c.category_id = c2s.category_id) LEFT JOIN " . DB_PREFIX  . "product_to_category p2c ON c.category_id=p2c.category_id LEFT JOIN " . DB_PREFIX  . "product p ON p.product_id=p2c.product_id WHERE p.manufacturer_id = '" . (int)$manufacturer_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND c2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND c.status = '1' AND p.status='1' AND (c.category_id IN (select path_id FROM oc_category_path GROUP BY path_id HAVING COUNT(category_id)=1) ) " );
        //$query = $this->db->query("SELECT DISTINCT c.category_id AS category_id,cd.name AS name FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) LEFT JOIN " . DB_PREFIX . "category_to_store c2s ON (c.category_id = c2s.category_id) LEFT JOIN " . DB_PREFIX  . "product_to_category p2c ON c.category_id=p2c.category_id LEFT JOIN " . DB_PREFIX  . "product p ON p.product_id=p2c.product_id WHERE p.manufacturer_id = '" . (int)$manufacturer_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND c2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND c.status = '1' AND (c.category_id IN (SELECT path_id FROM " .  DB_PREFIX . "category_path WHERE level=0 ) ) " );
        return $query->rows;
    }
    /* added by it-lab end */

    public function getCatalogTree() {
        $allCategories = $this->db->query("SELECT * FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) LEFT JOIN " . DB_PREFIX . "category_to_store c2s ON (c.category_id = c2s.category_id) WHERE information = 0 AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND c2s.store_id = '" . (int)$this->config->get('config_store_id') . "'  AND c.status = '1' ORDER BY c.sort_order, LCASE(cd.name)")->rows;
        $tree = [];
