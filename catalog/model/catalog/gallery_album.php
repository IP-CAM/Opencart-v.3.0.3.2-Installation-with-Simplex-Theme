...
class ModelCatalogGalleryAlbum extends Model
{
    ...
    {
        $sql = "SELECT * FROM " . DB_PREFIX . "gallery_album_images gai  LEFT JOIN " . DB_PREFIX . "gallery_album ga ON (gai.gallery_album_id = ga.gallery_album_id)  WHERE gai.language_id = '" . (int)$this->config->get('config_language_id') . "' AND ga.gallery_album_id='" . (int)$gallery_album_id . "'". " ORDER BY gai.sort_order DESC";
        $query = $this->db->query($sql);
        return $query->rows;
    }
    ...
    public function getRandomAlbum($gallery_album_id){
        $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "gallery_album WHERE gallery_album_id <> '" . (int)$gallery_album_id . "' ORDER BY RAND()");
    ...
    {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "gallery_album_images WHERE gallery_album_id='" . (int)$gallery_album_id . "' ". " AND `language_id` = '" . (int)$this->config->get('config_language_id')."'");
