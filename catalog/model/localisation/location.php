...
class ModelLocalisationLocation extends Model {
	public function getLocation($location_id) {
		$query = $this->db->query("SELECT location_id, name, address, geocode, telephone, telephone1, telephone2,email, fax, image, open, comment FROM " . DB_PREFIX . "location WHERE location_id = '" . (int)$location_id . "'");
    ...
    public function getLocations() {
        $query = $this->db->query("SELECT location_id, name, address, geocode, telephone, telephone1, telephone2, email, fax, image, open, comment, is_online FROM " . DB_PREFIX . "location" . " ORDER BY location_order");
