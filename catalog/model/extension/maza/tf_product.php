...
class ModelExtensionMazaTfProduct extends Model {
	public function getProducts($data = array()) {
                ...
                if(isset($data['filter_special']) || !empty($data['filter_min_price']) || !empty($data['filter_max_price'])){
                        ...
                        if(in_array('p.price', $sort)){
                            $sql .= ", (SELECT price FROM " . DB_PREFIX . "product_discount WHERE product_id = p.product_id AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND quantity = '1' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) GROUP BY product_id ORDER BY priority ASC, price ASC) AS discount";
                            $sql .= ", (SELECT price FROM " . DB_PREFIX . "product_special WHERE product_id = p.product_id AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((date_start = '0000-00-00 00:00:00' OR date_start < NOW()) AND (date_end = '0000-00-00 00:00:00' OR date_end > NOW())) GROUP BY product_id  ORDER BY priority ASC, price ASC) AS special";
                        }
                ...
                if($product_table == DB_PREFIX . 'product'){
                    ...
                    if(in_array('price', $additional_field) || !empty($data['filter_min_price']) || !empty($data['filter_max_price']) || isset($data['filter_special'])){
                        if($this->config->get('config_tax')){
                            ...
                            $sql .= ", (SELECT price + IFNULL(ftax.total,0) + ((IFNULL(ptax.total,0) * price) / 100) FROM " . DB_PREFIX . "product_discount WHERE product_id = p.product_id AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND quantity = '1' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) GROUP BY product_id ORDER BY priority ASC, price ASC) AS discount";
                            $sql .= ", (SELECT price + IFNULL(ftax.total,0) + ((IFNULL(ptax.total,0) * price) / 100) FROM " . DB_PREFIX . "product_special WHERE product_id = p.product_id AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((date_start = '0000-00-00 00:00:00' OR date_start < NOW()) AND (date_end = '0000-00-00 00:00:00' OR date_end > NOW())) GROUP BY product_id  ORDER BY priority ASC, price ASC) AS special";
                        } else {
                            $sql .= ", p.price";
                            $sql .= ", (SELECT price FROM " . DB_PREFIX . "product_discount WHERE product_id = p.product_id AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND quantity = '1' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) GROUP BY product_id ORDER BY priority ASC, price ASC) AS discount";
                            $sql .= ", (SELECT price FROM " . DB_PREFIX . "product_special WHERE product_id = p.product_id AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((date_start = '0000-00-00 00:00:00' OR date_start < NOW()) AND (date_end = '0000-00-00 00:00:00' OR date_end > NOW())) GROUP BY product_id  ORDER BY priority ASC, price ASC) AS special";
