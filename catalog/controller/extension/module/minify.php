...
class ControllerExtensionModuleMinify extends Controller {
    ...
    private function out($buffer) {
        $string = '';
        $string .= $this->status['js'] && !$this->status['async'] ? '<script type="text/javascript">var minify_out={};</script><script src="/' . $this->output_js . '" type="text/javascript"></script>' : '';
        $string .= $this->status['css'] ? '<link href="/' . $this->output_css . '" type="text/css" rel="stylesheet" />' : '';
        $buffer = str_replace('</head>', $string . '</head>', $buffer);

        if ($this->status['async']) {
            $buffer = str_replace('</body>','<script type="text/javascript">var minify_out={};</script></body>', $buffer);
