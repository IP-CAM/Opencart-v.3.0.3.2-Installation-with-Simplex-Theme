...
class ControllerProductProduct extends Controller
{
    ...
    public function index()
    {
        ...
        if ($product_info) {
            ...
            foreach ($downloads as $download) {
                ...
                $data['downloads'][] = array(
                    ...
                    'href' => $this->url->link(
                        ...
                        'product_id=' . $this->request->get['product_id'] . '&download_id=' . $download['download_id']
                    ),
            ...
            if ((float)$product_info['special']) {
                ...
                $data['economy'] = $this->tax->calculate(
                        $product_info['price'],
                        $product_info['tax_class_id'],
                        $this->config->get('config_tax')
                    ) - $this->tax->calculate(
            ...
            if ($this->config->get('config_tax')) {
                $data['tax'] = $this->currency->format(
                    (float)$product_info['special'] ? $product_info['special'] : $product_info['price'],
                    $this->session->data['currency']
                );
            } else {
                $data['tax'] = false;
                ...
                if ($result['image']) {
                    $image = $this->model_tool_image->resize(
                        ...
                        $this->config->get(
                            'theme_' . $this->config->get(
                                'config_theme'
                            ) . '_image_related_width'
                        ),
                        $this->config->get(
                            'theme_' . $this->config->get(
                                'config_theme'
                            ) . '_image_related_height'
                        )
                    );
                } else {
                    $image = $this->model_tool_image->resize(
            ...
            /* added by it-lab start */
            if ($category_info) {
                $results = $this->model_catalog_product->getRandomProductsFromCategory(
    ...
    public function write()
    {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            if ((utf8_strlen($this->request->post['text']) < 25) || (utf8_strlen(
